 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Construction_Field_Gallery' ) ) {

    class Construction_Field_Gallery extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                    ...
                    esc_html__('AT Gallery Section', 'construction-field'),
                    /*Widget description*/
                    array(
                            'description' => esc_html__( 'Show Gallery Section.', 'construction-field' )
                    )
            );
        }
        ...
        public function form( $instance ) {
            ...
            <br/>
            <small><?php esc_html_e( 'Add Page, Reorder and Remove. Please do not forget to add Featured Image on selected pages. ', 'construction-field' ); ?></small>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        $construction_field_image_sizes             = construction_field_get_image_sizes_options();
	        $instance[ 'construction_field_img_size' ]  = construction_field_sanitize_choice_options( $new_instance[ 'construction_field_img_size' ], $construction_field_image_sizes, 'large' );

	        $construction_field_gallery_image_popup     = construction_field_gallery_image_popup();
	        $instance[ 'image_popup_type' ]     = construction_field_sanitize_choice_options( $new_instance[ 'image_popup_type' ], $construction_field_gallery_image_popup, 'gallery' );
            ...
            <section id="<?php echo $unique_id;?>" class="at-widgets acme-gallery">
                <div class="full-width-container">
                    ...
                    if( !empty ( $post_in ) ) :
	                    ...
	                    $gallery_query = new WP_Query( $construction_field_post_in_page_args );
                        ?>
                        <div class="row fullwidth-row">
                            ...
                            if ( $gallery_query->have_posts() ):
                                ...
                                while( $gallery_query->have_posts() ):$gallery_query->the_post();
	                                ...
	                                if( 1 == $column_number ){
		                                $construction_field_column = " col-xs-12";
	                                }
                                    elseif( 2 == $column_number ){
		                                $construction_field_column = " col-xs-6";
	                                }
                                    elseif( 3 == $column_number ){
		                                $construction_field_column = " col-xs-12 col-md-4";
	                                }
                                    elseif( 4 == $column_number ){
		                                $construction_field_column = ' col-xs-12 col-md-3';
	                                }
	                                else{
		                                $construction_field_column = " col-xs-12 col-md-3";
	                                }
                                    ?>
                                    <div class="at-gallery-item <?php echo esc_attr( $construction_field_column ); ?>">
                                        <div class="gallery-inner-item <?php echo esc_attr( $animation1 );?>">
                                            ...
                                            if( has_post_thumbnail() ):
                                                $image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), $construction_field_img_size );
                                            else:
    ...
    } // Class Construction_Field_Gallery ends here
}