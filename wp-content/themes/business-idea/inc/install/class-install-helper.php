<?php
/**
 * Class business_idea_Plugin_Install_Helper
 */
class business_idea_Plugin_Install_Helper {
	/**
	 *
	...
	/**
	 *
	 * @return business_idea_Plugin_Install_Helper
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof business_idea_Plugin_Install_Helper ) ) {
			self::$instance = new business_idea_Plugin_Install_Helper;
		}
		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				case 'install':
					...
					$button .= '<a data-slug="' . $slug . '" class="install-now business_idea-install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="Install ' . $slug . '">' . __( 'Install and activate', 'business-idea' ) . '</a>';
					break;
				...
				case 'activate':
					...
					$button .= '<a data-slug="' . $slug . '" class="activate-now button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . __( 'Activate', 'business-idea' ) . '</a>';
					break;
			}
			$button .= '</div>';
		}// End if().
