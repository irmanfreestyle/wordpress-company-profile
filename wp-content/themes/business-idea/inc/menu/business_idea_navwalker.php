...
class business_idea_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$atts = array();
            ...
            } elseif( in_array( 'Businessidea-mm-heading', $item->classes ) && ( $item->url === '#' ) ) {
            	$item_output .= '<span class="mm-heading-wrapper">';
            } elseif( in_array( 'Businessidea-mm-heading', $item->classes ) ) {
			...
			if( ! empty( $item->description ) && ( $item->description !== ' ' ) && $depth >= 1 ) {
				$item_output .= '<li class="Businessidea-mm-description">' . $item->description;
			}
