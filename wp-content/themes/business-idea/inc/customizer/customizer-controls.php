...
class business_idea_Theme_Color_Customize_Control extends WP_Customize_Control {
public $type = 'new_menu';

       function render_content(){
		   
	   echo '<h3>'._e('Primary Color','business-idea').'</h3>';
		  ...
		  <script>
			jQuery(document).ready(function($) {
				$("#customize-control-business_idea_option-theme_color label div").click(function(){
					$("#customize-control-business_idea_option-theme_color label div").removeClass("color_active");
...
class business_idea_Section_Plus extends WP_Customize_Section {

    public $type = 'business_idea-pro';
...
class business_idea_Misc_Control extends WP_Customize_Control {
...
class business_idea_Textarea_Custom_Control extends WP_Customize_Control
{
	public function render_content() {
		?>
		<label>
...
class business_idea_Theme_Support extends WP_Customize_Control {
	public function render_content() {
		echo wp_kses_post( 'Upgrade to <a href="#">business_idea Plus</a> to be able to change the section order and styling!', 'business-idea' );
	}
}

class business_idea_Editor_Custom_Control extends WP_Customize_Control
{
...
class business_idea_Alpha_Color_Control extends WP_Customize_Control {
...
class business_idea_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'business_idea_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
            $this->max_item = absint( $args['max_item'] );
        }

        $this->changeable =  isset(  $args['changeable'] ) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset(  $args['default_empty_title'] ) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__( 'Item', 'business-idea' );
    ...
    public function render_content() {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php echo ( $this->add_text ) ? esc_html( $this->add_text ) : __( 'Add an item', 'business-idea' ); ?></span>
        </div>
        <?php
    }
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php _e( 'Item', 'business-idea' ); ?></h4>
                    </div>
                </div>
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    #>
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php _e( 'Remove', 'business-idea' ) ?></button>
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'business-idea' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'business-idea' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add', 'business-idea' ); ?> <# } else { #> <?php _e( 'Change', 'business-idea' ); ?> <# } #> </button>
                                        ...
                                        <# }  else if ( field.type == 'icon'  ) { #>
                                            ...
                                            <a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'business-idea' ); ?></a>
                                        <# }  #>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php _e( 'Remove', 'business-idea' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php _e( 'Close', 'business-idea' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
...
class business_idea_Category_Control extends WP_Customize_Control {
    ...
    protected function render_content() {
        ...
        $dropdown_args = wp_parse_args( $this->dropdown_args, array(
            ...
            'show_option_none'  => '',
            'selected'          => $this->value(),
            'show_option_all'   => __( 'All', 'business-idea' ),
            'orderby'           => 'id',
            'order'             => 'ASC',
            'show_count'        => 1,
            'hide_empty'        => 1,
...
function business_idea_enqueue_editor(){
...
class business_idea_Editor_Scripts
{
...
function business_idea_customizer_control_scripts(){
    wp_enqueue_media();
    wp_enqueue_script( 'jquery-ui-sortable' );
    wp_enqueue_script( 'wp-color-picker' );
    wp_enqueue_style( 'wp-color-picker' );

    wp_enqueue_script( 'business_idea-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-controls', 'wp-color-picker' ) );
    wp_enqueue_style( 'business_idea-customizer',  get_template_directory_uri() . '/assets/css/customizer.css' );

}

add_action( 'customize_controls_enqueue_scripts', 'business_idea_customizer_control_scripts', 99 );
add_action( 'customize_controls_enqueue_scripts', array( 'business_idea_Editor_Scripts', 'enqueue' ), 95 );